...
class _MapScreenState extends State<MapScreen> {
  ...
  static final List<FilterOption> _filters = [
    ...
    FilterOption(key: 'restroom', icon: Icons.wc, label: 'Restrooms', hasBottomSheet: true),
  ];
  ...
  Future<void> _loadBuildingBathrooms() async {
    try {
      ...
      for (var feature in features) {
        ...
        if (properties['feature_type'] == 'building') {
          ...
          try {
            if (type == 'Point') {
              lng = (coords[0] as num).toDouble(); lat = (coords[1] as num).toDouble();
            } else if (type == 'Polygon') {
              lng = (coords[0][0][0] as num).toDouble(); lat = (coords[0][0][1] as num).toDouble();
            } else if (type == 'MultiPolygon') {
              lng = (coords[0][0][0][0] as num).toDouble(); lat = (coords[0][0][0][1] as num).toDouble();
            }
          } catch (e) { continue; }
          
          if (lat != 0.0) {
            _bathroomPinOptions.add(PointAnnotationOptions(
    ...
    } catch (e) { debugPrint("Error loading buildings"); }
  }

  void _onMapCreated(MapboxMap map) async {
    mapboxMap = map;
    await mapboxMap!.gestures.updateSettings(GesturesSettings(rotateEnabled: false, pitchEnabled: false));
    await mapboxMap!.setCamera(CameraOptions(bearing: 0, center: Point(coordinates: Position(_centerLng, _centerLat)), zoom: 15.0));
    await mapboxMap!.setBounds(CameraBoundsOptions(bounds: campusBounds, minZoom: 13.0, maxZoom: 20.0));
    await mapboxMap!.location.updateSettings(LocationComponentSettings(enabled: true, pulsingEnabled: true));
  ...
  void _onMapTapped(MapContentGestureContext context) async {
    ...
    _devCircle = await _devCircleManager?.create(CircleAnnotationOptions(
      geometry: context.point, circleRadius: 9.0, circleColor: const Color(0xFF4285F4).value, circleStrokeWidth: 3.0, circleStrokeColor: Colors.white.value,
    ));
    _devPin = await _devPinManager?.create(PointAnnotationOptions(
      geometry: context.point, textField: "You (Simulated)", textColor: Colors.black.value, textHaloColor: Colors.white.value, textHaloWidth: 2.0, textOffset: [0.0, 1.2], 
    ));
    setState(() {});
  }
  ...
  void _locateUser() {
    if (_isDevMode && _simulatedPosition != null) {
      mapboxMap?.flyTo(CameraOptions(center: Point(coordinates: _simulatedPosition!), zoom: 17.0), MapAnimationOptions(duration: 500));
    } else {
  ...
  void _resetView() {
    mapboxMap?.flyTo(CameraOptions(center: Point(coordinates: Position(_centerLng, _centerLat)), zoom: 15.0, pitch: 0.0, bearing: 0.0), MapAnimationOptions(duration: 1000));
  }

  void _onFilterTapped(FilterOption filter) {
    setState(() { _activeFilter = (_activeFilter == filter.key) ? null : filter.key; });
    _updateMapPins();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: AppBar(
          backgroundColor: const Color(0xFFF2D21B), elevation: 0, titleSpacing: 16, toolbarHeight: 300,
          title: Container(
            padding: const EdgeInsets.all(5), color: const ui.Color.fromARGB(255, 239, 236, 227),
            child: Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Switch(
                      value: _isDevMode, activeColor: Colors.red,
                      onChanged: (val) {
      ...
      bottomSheet: sheetContent != null
          ? _FilterBottomSheet(child: sheetContent, onClose: () { setState(() => _activeFilter = null); _updateMapPins(); })
          : null,
      body: Stack(
        children: [
          MapWidget(
              key: const ValueKey("mapWidget"), styleUri: "mapbox://styles/theresa2/cmlbykdmm000s01su4z139emu", textureView: true,
              onMapCreated: _onMapCreated, onTapListener: _onMapTapped, 
          ),
          Positioned(
            top: 20, bottom: 370, right: 12,
            child: ScrollConfiguration(
              behavior: ScrollConfiguration.of(context).copyWith(scrollbars: false),
              child: SingleChildScrollView(
                physics: const BouncingScrollPhysics(),
                child: Column(children: _filters.map((f) => Padding(
                  padding: const EdgeInsets.only(bottom: 10),
                  child: RawMaterialButton(
                    onPressed: () => _onFilterTapped(f),
                    fillColor: _activeFilter == f.key ? const Color(0xFFFFCC00) : const ui.Color.fromARGB(255, 243, 250, 255),
                    shape: const CircleBorder(),
                    constraints: const BoxConstraints.tightFor(width: 55, height: 55),
                    elevation: 4,
                    child: Icon(f.icon, color: Colors.black87, size: 30),
                  ),
                )).toList()),
              ),
            ),
...
class _FilterBottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DraggableScrollableSheet(
      initialChildSize: 0.35, minChildSize: 0.35, maxChildSize: 0.90, expand: false, snap: true, snapSizes: const [0.35, 0.90],
      builder: (context, scrollController) {
        return Container(
          decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.vertical(top: Radius.circular(16)), boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 8, offset: Offset(0, -2))]),
          child: ListView(
            ...
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    const Expanded(child: Center(child: SizedBox(width: 40, height: 5, child: DecoratedBox(decoration: BoxDecoration(color: Color(0xFFDDDDDD), borderRadius: BorderRadius.all(Radius.circular(5))))))),
...
class _FoodAlertSheetContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Padding(padding: EdgeInsets.only(left: 20, top: 0, bottom: 5), child: Text("Food Alerts", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87))),
        ListView.separated(
          shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.only(top: 5, bottom: 20), itemCount: 3,
          separatorBuilder: (context, index) => Divider(color: Colors.grey.shade200, height: 1),
          itemBuilder: (context, index) {
            return ListTile(
              contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 2),
              leading: Container(padding: const EdgeInsets.all(10), decoration: const BoxDecoration(color: Color(0xFFE8F0FE), shape: BoxShape.circle), child: const Icon(Icons.pin_drop, color: Colors.red, size: 22)),
              title: const Text("Recent Alert", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              subtitle: const Text("Details coming soon", style: TextStyle(color: Colors.grey, fontSize: 13)),
...
class _RestroomSheetContentState extends State<_RestroomSheetContent> {
  ...
  Future<void> _loadList() async {
    try {
      ...
      for (var feature in features) {
        ...
        double dist = _getDistance(widget.currentPosition.lat.toDouble(), widget.currentPosition.lng.toDouble(), lat, lng);
        results.add({
          'name': props['name'], 'abbrev': props['abbrev'], 'distVal': dist,
          'distStr': dist < 1000 ? "${dist.toStringAsFixed(0)} ft" : "${(dist / 5280).toStringAsFixed(1)} mi",
      ...
      if (mounted) setState(() { _dynamicList = top5; _isLoading = false; });

      for (var b in top5) {
        String id = b['abbrev'] != 'None' ? b['abbrev'] : b['name'];
        final snap = await FirebaseFirestore.instance.collection('bathroom_reviews')
            .where('bathroomName', isGreaterThanOrEqualTo: id).where('bathroomName', isLessThanOrEqualTo: '$id\uf8ff').get();

        if (snap.docs.isNotEmpty) {
          Map<String, int> counts = {};
          for (var doc in snap.docs) {
            Map<String, dynamic> fts = doc.data()['features'] ?? {};
            fts.forEach((k, v) { if (v == true) counts[k] = (counts[k] ?? 0) + 1; });
          }
          if (counts.isNotEmpty && mounted) {
            setState(() { b['details'] = counts.entries.reduce((a, b) => a.value > b.value ? a : b).key; });
          } else if (mounted) {
            setState(() { b['details'] = "Functional"; });
          }
        } else if (mounted) {
          setState(() { b['details'] = "No reviews yet"; });
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Padding(padding: EdgeInsets.only(left: 20, top: 0, bottom: 5), child: Text("Nearby Restrooms", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87))),
        if (_isLoading) const Padding(padding: EdgeInsets.all(40), child: Center(child: CircularProgressIndicator())),
        if (!_isLoading) ListView.separated(
          shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.only(top: 5, bottom: 20), itemCount: _dynamicList.length,
          separatorBuilder: (context, index) => Divider(color: Colors.grey.shade200, height: 1),
          itemBuilder: (context, index) {
            final b = _dynamicList[index];
            return ListTile(
              contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 2),
              leading: Container(padding: const EdgeInsets.all(10), decoration: const BoxDecoration(color: Color(0xFFE8F0FE), shape: BoxShape.circle), child: const Icon(Icons.wc, color: Colors.blueAccent, size: 22)),
              title: Text(b["name"]!, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 16)),
              subtitle: Text("${b["distStr"]} - ${b["details"]}", style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
              trailing: const Icon(Icons.chevron_right, color: Colors.grey),
              onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const BathroomFinder())),
            );
          },
        ),
      ],
...
class _SheetPlaceholder extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Center(child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: Colors.black54, size: 20), const SizedBox(width: 8), Text(label, style: const TextStyle(color: Colors.black54, fontSize: 14))]));
  }
}
